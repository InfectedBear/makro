...
with TelegramClient(sesi_file, api_id, api_hash) as client:
    client.loop.run_until_complete(client.send_message('KampungMaifamXBot', mesej))
    ...
    async def handler(event):
        ...
        if 'Pemerintah Maikantri' in event.raw_text:
        #if 'perempuan' in event.raw_text:
            mezo = await client.get_messages('KampungMaifamXBot', ids=event.message.id)
            time.sleep(2)
